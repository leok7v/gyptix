...
static int load_session(struct state &state, std::string filename) {
...
//  params.interactive_first     = false; // it will be modified later...
    ...
    if (state.params.conversation_mode == COMMON_CONVERSATION_MODE_AUTO) {
        if (state.has_chat_template) {
            trace("chat template is available, enabling conversation mode (disable it with -no-cnv)\n");
    ...
    if (state.params.conversation_mode && !state.has_chat_template) {
        trace("chat template is not available or is not supported. This may cause the model to output suboptimal responses\n");
    }
