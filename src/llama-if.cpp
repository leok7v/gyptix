...
//  params.interactive_first     = false; // it will be modified later...
    ...
    n_ctx = llama_n_ctx(state.ctx);
    int &n_ctx_train = state.n_ctx_train;
    n_ctx_train = llama_model_n_ctx_train(state.model);
    ...
    if ((int) state.embd_inp.size() > n_ctx - 4) {
        trace("prompt is too long (%d tokens, max %d)\n",
              (int)state.embd_inp.size(), n_ctx - 4);
        return 1;
    }
    ...
    if (state.params.n_keep < 0 || state.params.n_keep > (int) state.embd_inp.size()) {
...
//      GGML_ASSERT(n_ctx >= n_ctx_train * ga_n && "n_ctx must be at least n_ctx_train * grp_attn_n"); // NOLINT
    ...
    for (const std::string & antiprompt : state.params.antiprompt) {
        antiprompt_ids.emplace_back(::common_tokenize(state.ctx, antiprompt, false, true));
    }
    if (llama_model_has_encoder(state.model)) {
            ...
            if (ga_n == 1) {
                ...
                if (state.n_past + (int) state.embd.size() >= n_ctx) {
                    ...
                    LOG_DBG("context full, swapping: n_past = %d, n_left = %d, n_ctx = %d, n_keep = %d, n_discard = %d\n",
                            state.n_past, n_left, n_ctx, state.params.n_keep, n_discard);
...
//              trace("saving %d tokens to '%s'\n", (int)session_tokens.size(), path_session.c_str());
                llama_state_save_file(state.ctx, path_session.c_str(), session_tokens.data(), session_tokens.size());
...
//                  output_ss << token_str;
        ...
        if ((int)state.embd_inp.size() <= state.n_consumed) {
            ...
            if (is_eog) {
                ...
                if (state.params.interactive) {
                    ...
                    if (state.params.enable_chat_template) {
                        add_and_format(state, "assistant", assistant_ss.str());
                    }
            ...
            if (state.params.conversation_mode) {
                ...
                assistant_ss << common_token_to_piece(state.ctx, id, false);
            }
            if (interrupted) {
...
//                  trace("ENDS RUNNING THE MODEL\n");
                ...
                if (buffer.length() > 1) {
                    ...
                    if (!state.params.input_suffix.empty() && !state.params.conversation_mode) {
...
//                  trace("at_readline.embd_inp_size: %d\n", (int)context.at_readline.embd_inp_size);
                    ...
                    bool format_chat = state.params.conversation_mode && state.params.enable_chat_template;
