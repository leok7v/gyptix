...
struct Gyptix: App {
    ...
    init() {
        UserDefaults.standard.set(is_debugger_attached(), forKey: "WebKitDeveloperExtras")
    }
    ...
    private func restrictWindows() {
        ...
        if let w = NSApplication.shared.windows.first {
            w.performSelector(onMainThread: #selector(NSWindow.toggleTabBar(_:)), with: nil, waitUntilDone: false)
...
func close_all_windows() {
    #if os(macOS)
    DispatchQueue.main.async { NSApplication.shared.windows.forEach { $0.close() } }
    #endif
}
